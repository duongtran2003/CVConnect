...
watch(
  ...
  async (newVal) => {
    if (
      newVal &&
      props.data &&
      props.data.mailTemplate &&
      props.data.mailTemplate.mailTemplate
    ) {
      setLoading(true);
      const payload = {
        positionName: props.data.generalInfo.position
          ? props.data.generalInfo.position.label
          : undefined,
        jobAdName: props.data.generalInfo.title.trim()
          ? props.data.generalInfo.title.trim()
          : undefined,
        orgName: orgInfo.value?.name ?? undefined,
        candidateName: "Nguyễn Minh Quân",
        hrContactId: props.data.hrInfo.hrInfo
          ? props.data.hrInfo.hrInfo.value
          : undefined,
      };
      const res = await getMailTemplatePreviewDefault(
        props.data.mailTemplate.mailTemplate.value,
        payload,
      );
...
watch(
  ...
  async (newVal) => {
    ...
    if (newVal && newVal.mailTemplate && newVal.mailTemplate.value) {
      setLoading(true);
      const payload = {
        positionName: newVal.position
          ? newVal.position.name
          : undefined,
        jobAdName: newVal.title.trim()
          ? newVal.title.trim()
          : undefined,
        orgName: orgInfo.value?.name ?? undefined,
        candidateName: "Nguyễn Minh Quân",
        hrContactId: newVal.hrContact
          ? newVal.hrContact.id
          : undefined,
      };
      const res = await getMailTemplatePreviewDefault(
